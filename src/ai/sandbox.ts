...
export async function reviewArticle(input: {
  ...
  const { text, usage } = await generateText({
    ...
    system:
      ...
      "【评分校准·重要】qualityScore 必须覆盖全区间，禁止默认给高分，多数普通文章应落在 0.4-0.7。" +
      "锚点示例：0.1-0.2=纯导航/标签云/无实质内容；0.3=简短动态/一句话快讯，信息量低；" +
      "0.5=中等相关、信息有限；0.7=有具体事件与数据、时效性好；0.9=重大突破/独家/高影响力。" +
      "避免锚定偏差：不要因为'它是新闻'就给 0.8+，要按实际信息密度打分。" +
